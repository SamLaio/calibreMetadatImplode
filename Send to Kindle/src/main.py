...
class ProcessDialog(QDialog):
    ...
    def get_changed_aname(self, name, aname):
        template = JSONConfig('mtp_devices').get('send_template', '{title} - {authors}')
        for item in self.ebooks.values():
            if name == item[1].lower():
                mi = self.gui.current_db.new_api.get_metadata(item[0])
                mi.title = item[2]
                # Preserve Unicode from calibre settings; do not ASCII-ify or transliterate.
                path = create_upload_path(
                    mi, aname, template, lambda x: x,
                    prefix_path='',
                    path_type=posixpath,
                    maxlen=255,
                    use_subdirs='/' in template,
