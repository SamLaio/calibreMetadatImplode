...
class SplitEpub:
    ...
    def get_content_dom(self):
        if not self.content_dom:
            ...
            try:
                version = self.content_dom.documentElement.getAttribute("version").strip()
                self.source_version_major = int(version.split('.')[0])
            except:
                self.source_version_major = 2
    ...
    def get_toc_relpath(self):
        ## Save the path to the TOC file--hrefs inside it are relative to it.
        if not self.toc_relpath:
            self.get_manifest_items() # sets self.toc_relpath also.
        return self.toc_relpath

    def get_nav_relpath(self):
        ## Save the path to the nav.xhtml file--hrefs inside it are relative to it.
        if not self.nav_relpath:
            self.get_manifest_items() # sets self.nav_relpath also.
        return self.nav_relpath
        ...
        if not self.manifest_items:
            ...
            for item in self.get_content_dom().getElementsByTagName("item"):
                ...
                if 'nav' in properties:
                    self.nav_relpath = get_path_part(fullhref)
                    self.nav_dom = parseString(self.epub.read(fullhref))
                    self.toc_relpath = self.nav_relpath
                    self.toc_kind = 'nav'
                elif( item.getAttribute("media-type") == "application/x-dtbncx+xml" and self.toc_kind != 'nav' ):
    ...
    def _append_toc_map_entry(self, href, text, anchor):
    ...
    def _get_nav_toc_entries(self):
        entries = []
        navdom = self.get_toc_dom()
        if navdom is None:
            return entries
        soup = make_soup(navdom.toxml())
        tocnav = None
        for nav in soup.find_all('nav'):
            etype = nav.get('epub:type', '') or ''
            role = nav.get('role', '') or ''
            if 'toc' in etype.split() or 'doc-toc' in role:
                tocnav = nav
                break
        if tocnav is None:
            tocnav = soup.find('nav')
        if tocnav is None:
            return entries
        for a in tocnav.find_all('a', href=True):
            src = normpath(unquote(self.get_toc_relpath()+a['href']))
            if '#' in src:
                (href,anchor)=src.split("#", 1)
            else:
                (href,anchor)=(src,None)
            text = a.get_text(" ", strip=True)
            entries.append((href, text, anchor))
        return entries
    ...
    def get_toc_map(self):
        if not self.toc_map:
            self.toc_map = {}
            if self.toc_kind == 'nav':
                for href, text, anchor in self._get_nav_toc_entries():
                    self._append_toc_map_entry(href, text, anchor)
            else:
                tocdom = self.get_toc_dom()
                if tocdom is None:
                    return self.toc_map
    ...
    def _get_nav_xhtml(self, titleopt, changedtocs, language):

        navdom = getDOMImplementation().createDocument("http://www.w3.org/1999/xhtml", "html", None)
        ...
        head = navdom.createElement("head")
        html.appendChild(head)
        head.appendChild(newTag(navdom, "meta", {"charset": "utf-8"}))
        head.appendChild(newTag(navdom, "title", text=stripHTML(titleopt)))

        body = navdom.createElement("body")
        html.appendChild(body)
        nav = navdom.createElement("nav")
        nav.setAttribute("epub:type", "toc")
        nav.setAttribute("id", "toc")
        body.appendChild(nav)
        nav.appendChild(newTag(navdom, "h2", text="Contents"))

        ol = navdom.createElement("ol")
        nav.appendChild(ol)
    ...
    def write_split_epub(self,
        ...
        is_epub3 = self.source_version_major >= 3
...
####    ## create content.opf file.
        ...
        package.setAttribute("version","3.0" if is_epub3 else "2.0")
        package.setAttribute("xmlns","http://www.idpf.org/2007/opf")
        package.setAttribute("unique-identifier","epubsplit-id")
        if is_epub3:
            package.setAttribute("prefix", "dcterms: http://purl.org/dc/terms/")
        ...
        spine = newTag(contentdom,"spine",attrs={"toc":"ncx"} if not is_epub3 else None)
        ...
        if is_epub3:
            navdom = self._get_nav_xhtml(titleopt, changedtocs, language)
            outputepub.writestr("nav.xhtml", navdom.toprettyxml(indent='   ', encoding='utf-8'))
